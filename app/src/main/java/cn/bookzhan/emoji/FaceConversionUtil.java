...
public class FaceConversionUtil {
	...
	public SpannableString addFace(Context context, int imgId,
		...
		Bitmap bitmap = BitmapFactory.decodeResource(context.getResources(),
				imgId);
		bitmap = Bitmap.createScaledBitmap(bitmap, DisplayUtil.dip2px(context, 23), DisplayUtil.dip2px(context,23), true);
	...
	private void dealExpression(Context context,
		...
		while (matcher.find()) {
			...
			if (resId != 0) {
				...
				bitmap = Bitmap.createScaledBitmap(bitmap,DisplayUtil.dip2px(context,21), DisplayUtil.dip2px(context,21), true);
	...
	private List<ChatEmoji> getData(int page) {
		...
		if (list.size() == pageSize) {
			ChatEmoji object = new ChatEmoji();
			object.setId(R.drawable.selector_face_del_icon);
			list.add(object);
		}
		return list;
	}
